...
(function () {
    ...
    style.textContent = `
        ...
        @media (max-width: 800px) { .custom-profile-dashboard { flex-direction: column; gap: 16px; height: auto !important; padding: 16px !important; align-items: flex-start; } .cpd-stats-actions-col { height: auto !important; border-left: none !important; border-right: none !important; border-top: 1px solid #e0e0e0; padding-left: 0 !important; padding-right: 0 !important; padding-top: 20px; flex-direction: row; flex-wrap: wrap; width: 100%; justify-content: center; } .cpd-actions-block { width: 100%; } }
        ...
        .is-overview-page article.cardbox { height: 145px !important; box-sizing: border-box !important; }
        ...
        .is-overview-page .custom-ad-grid > div:first-child, .is-overview-page .custom-ad-grid > div:first-child > a { width: 170px !important; height: 110px !important; box-sizing: border-box !important; display: flex !important; align-items: center !important; justify-content: center !important; overflow: hidden !important; }
        .is-overview-page .custom-ad-grid > div:first-child img { width: 100% !important; height: 100% !important; object-fit: cover !important; }
        ...
        .custom-action-area button span, .custom-action-area a span, .is-detail-page .custom-native-btn-detail span, .is-detail-page .custom-purple-btn span { font-size: 12px !important; }
        ...
        .custom-analyse-grid { display: grid !important; grid-template-columns: minmax(101px, max-content) minmax(93px, max-content) max-content !important; column-gap: 8px !important; row-gap: 8px !important; width: 100% !important; margin-top: 4px !important; }
        ...
        #custom-top-pagination nav { margin: 0 !important; padding: 0 !important; width: auto !important; display: block !important; }
        #custom-top-pagination nav > ul { margin: 0 !important; }
    ...
    function showLoading(customMsg) {
        ...
        const msg = customMsg || 'Aktion wird ausgeführt...<br><span style="font-size: 14px; color: #666; font-weight: normal; margin-top: 10px; display: block;">Bitte klicke nichts an. Die Seite lädt gleich neu.</span>';
        spinnerContainer.innerHTML = `<div style="font-size: 20px; font-weight: bold; padding: 30px; background: white; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); color: #86B817; text-align: center;">${msg}</div>`;
        if (document.body) document.body.appendChild(spinnerContainer);
    ...
    if (isOverviewPage && postActionStr) {
        localStorage.removeItem('__KL_POST_ACTION');
    }

    const isSuccessPage = isConfirmPage || (isDetailPage && !window.location.href.includes('bearbeiten'));

    if (postActionStr && isSuccessPage) {
        document.documentElement.style.opacity = '0';

        window.addEventListener('DOMContentLoaded', () => {
            document.documentElement.style.opacity = '1';
            showLoading("Aktion erfolgreich!<br><span style='font-size:14px; font-weight:normal; color:#666;'>Alte Anzeige wird entfernt und du wirst weitergeleitet...</span>");
        });

        const postAction = JSON.parse(postActionStr);
        localStorage.removeItem('__KL_POST_ACTION');

        if (postAction.action === 'relist' && postAction.oldAdId) {
            (async () => {
                try {
                    let token = document.querySelector('meta[name="_csrf"]')?.content;
                    if (!token) {
                        const res = await fetch('/m-meine-anzeigen.html');
                        const html = await res.text();
                        const match = html.match(/<meta\s+name="_csrf"\s+content="([^"]+)"/i);
                        if (match) token = match[1];
                    }
                    if (token) {
                        await fetch(`/m-anzeigen-loeschen.json?ids=${postAction.oldAdId}`, {
                            method: 'POST', headers: { 'x-csrf-token': token }
                        });
                    } else {
                        await fetch(`/m-anzeige-loeschen.html?adId=${postAction.oldAdId}`);
                    }
                } catch(e) {
    ...
    const inject = () => {
                            ...
                            if (text.includes('drucken') || text.includes('Verkaufsschild')) {
                                ...
                                if (isDetailPage) {
                                    ...
                                    btn.addEventListener('click', () => {
                                        setTimeout(preventDisable, 10); setTimeout(preventDisable, 100); setTimeout(preventDisable, 500);
                                    });
        ...
        if (isEditPage) {
            ...
            const doAction = (e, type) => {
                ...
                localStorage.setItem('__KL_POST_ACTION', JSON.stringify({
                    action: type,
                    oldAdId: currentAdId,
                    timestamp: Date.now()
                }));
    ...
    if (isOverviewPage) {
        ...
        async function fetchAllUserAds() {
            try {
                ...
                while (true) {
                    if (pageNum === currentPage) { pageNum++; continue; }
                    ...
                    if (lastPage === null && data.paging && data.paging.last) lastPage = data.paging.last;
                    ...
                    data.ads.forEach(ad => {
                        ...
                        const imgSrc = ad.adImage ? (ad.adImage.url || ad.adImage.link || ad.adImage.src || '') : '';
                            ...
                            <div class="grid w-full custom-ad-grid">
                                <div class="self-start">
                                    <a href="${ad.seoUrl || '#'}" class="flex justify-center items-center rounded-small bg-surfaceSubdued overflow-hidden w-[200px] h-[150px] relative" aria-label="Anzeige ${cleanTitle.replace(/"/g, '&quot;')}" style="border-radius: 4px;">
                                        <img src="${imgSrc || 'https://www.kleinanzeigen.de/static/img/common/default_image_thumbnail.png'}" alt="${cleanTitle.replace(/"/g, '&quot;')}" class="w-full h-full object-cover imagebox-image" data-testid="ad-image">
                                        ${imgBadgeHtml}
                                    </a>
                                </div>
                                <div class="pl-medium align-top">
                                    ...
                                    <ul class="list">
                                        <li class="mx-none my-xsmall text-title3 text-secondary has-custom-btn">
                                            <div class="custom-price-left" style="display: flex; align-items: center; gap: 6px; flex-wrap: nowrap;">
                                                ${ad.price || ''}
                                                ${ad.shipping ? `<span class="custom-shipping-info" style="font-size: 12px; font-weight: normal; color: rgb(117, 117, 117);">${ad.shipping}</span>` : ''}
                                            </div>
                ...
                if (!masterToolbar) {
                    ...
                    pagContainer.addEventListener('click', (e) => {
                        ...
                        if (btn && btn.tagName === 'BUTTON') {
                            e.preventDefault(); e.stopPropagation();
                            const cloneButtons = Array.from(pagContainer.querySelectorAll('button'));
                            ...
                            if (realNavContainer) {
                                ...
                                if (realButtons[idx]) realButtons[idx].click();
                            }
                        }
                    });
                }
    ...
    if (isEditPage) {
        ...
        const configStr = localStorage.getItem('__KL_AUTO_ACTION');
        if (configStr) {
            try {
                ...
                if (config.adId === currentId) {
                    ...
                    window.addEventListener('load', () => {
                        showLoading();
                        setTimeout(() => {
                            const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Anzeige speichern') || b.textContent.includes('speichern'));
                            if (btn) {
                                localStorage.setItem('__KL_POST_ACTION', JSON.stringify({
    ...
    window.fetch = async function(...args) {
        ...
        if (actionStr) {
            try {
                ...
                if (Date.now() - actionData.timestamp < 15000) {
                    if (typeof args[0] === 'string' && args[0].includes('bearbeiten')) {
                        args[0] = args[0].replace('bearbeiten', 'aufgeben').replace(/adId=\d+/, '');
                    }
                    if (args[1]?.body) {
                        if (typeof args[1].body === 'string') {
                            args[1].body = args[1].body.replace(/adId=\d+/, '').replace(/"adId":\d+/, '');
                        } else if (args[1].body instanceof FormData) {
                            args[1].body.delete('adId');
                            args[1].body.delete('id');
                        }
                    }
                }
            } catch(e) {}
